...
if st.session_state.user_submitted:
    ...
    st.subheader("Enter your semester-wise course data")
    ...
    if st.button("Calculate CGPA and Print"):
        ...
        for sem in range(user['StartSem'], user['MaxSem'] + 1):
            ...
            sem_df = df[df["Semester"] == sem][["Credit", "Score", "Grade"]]
            output.write(sem_df.to_string(index=False))
            gpa = sem_stats.loc[sem_stats["Semester"] == sem, "GPA"].values[0]
            output.write(f"\nSemester GPA: {gpa:.2f}\n\n")

        output.write(f"{border}\nDisclaimer: CGPA was calculated based on the data fed by student.\n{border}\n")

        st.download_button(
            label="Download Report (Text Format)",
